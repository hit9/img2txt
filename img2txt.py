...
def generate_ANSI_from_pixels(pixels, width, height, bgcolor_rgba, get_pixel_func = None, is_overdraw = False):
    """Does not output final newline or reset to particular colors at end -- caller should do that if desired
       bgcolor_rgba=None is treated as default background color
    """
    ...
    for h in xrange(height):  
        for w in xrange(width):
            ...
            if not skip_pixel:
                ...
                if draw_char is None:
                    ...
                    # then just skip this. We need to make this check here because the conversion to ANSI can 
                    ...
                    # is already drawn at this location
                ...
                if not skip_pixel:
                    ...
                    # If we are not at the cursor x location (happens if we skip pixels) output sequence to get there
                    ...
                    if cursor_x < w:
                        ...
                        string += "\x1b[{0}C".format(w - cursor_x)  # code to advance N columns ahead
                        cursor_x = w
                    ...
                    if draw_char == " ":
                        ...
                        # No, we can't output an "upper ascii" character that fills the entire foreground - terminals
                        # don't display these the same way, if at all. 
        ...
        if (h + 1) != height:
            ...
            # to EOL with current bg color. However, if cursor had been moved up and this just goes back down to an existing 
...
def generate_HTML_for_image(pixels, width, height):
    ...
    string = ""
    for h in xrange(height):  # first go through the height,  otherwise will rotate
        for w in xrange(width):

            rgba = pixels[w, h]

            # TODO - could optimize output size by keeping span open until we hit end of line or different color/alpha
            #   Could also output just rgb (not rgba) when fully opaque - if fully opaque is prevalent in an image
...
def generate_grayscale_for_image(pixels, width, height, bgcolor):
    ...
    for h in xrange(height):  # first go through the height,  otherwise will rotate
        for w in xrange(width):
            ...
            # If partial transparency and we have a bgcolor, combine with bg color
            if rgba[3] != 255 and bgcolor is not None:
                rgba = alpha_blend(rgba, bgcolor)

            # Throw away any alpha (either because bgcolor was partially transparent or had no bg color)
            # Could make a case to choose character to draw based on alpha but not going to do that now...
            rgb = rgba[:3]  
...
if __name__ == '__main__':
    ...
    try:
        bgcolor = HTMLColorToRGB(bgcolor) + (255, )     # add fully opaque alpha value (255)
    except:
        bgcolor = None
    ...
    if ansi:
        ...
        # Since the "current line" was not established by us, it has been filled with the current background color in the
        # terminal. We have no ability to read the current background color so we want to refill the line with either
        # the specified bg color or if none specified, the default bg color.    
        if bgcolor is not None:
            # Note that we are making the assumption that the viewing terminal supports BCE (Background Color Erase) 
            # otherwise we're going to get the default bg color regardless. If a terminal doesn't support BCE you can
            # output spaces but you'd need to know how many to output (too many and you get linewrap) 
            fill_string = getANSIbgstring_for_ANSIcolor(getANSIcolor_for_rgb(bgcolor))
        else:
            fill_string = "\x1b[49m"     # reset bg to default (if we want to support terminals that can't handle this will need to instead use 0m which clears fg too and then when using this reset prior_fg_color to None too    
        fill_string += "\x1b[K"          # does not move the cursor
        sys.stdout.write(fill_string)

        sys.stdout.write(generate_ANSI_from_pixels(pixel, width, height, bgcolor))

        # Undo residual color changes, output newline because generate_ANSI_from_pixels does not do so
        sys.stdout.write("\x1b[0m\n")     # removes all attributes (formatting and colors)  
    else:

        if clr:
            # TODO - should handle bgcolor - probably by setting it as BG on the CSS for the pre
